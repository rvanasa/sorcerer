...
function find(callback)
{
	...
	for(var param of getParamNames(callback))
	{
		...
		else throw new Error('Resource not found: `' + param + '`');
	}
	Resource.all(resources, (values) => callback.apply(this, values));
}

function resolveResource(input, context, providers)
{
	...
	else
	{
		...
		{
			var value = input[id];
			context[id] = value instanceof Resource ? value : Resource.static(value);
		}
	}
}
...
function sorcerer()
{
	...
	{
		resolveResource(input, context, providers);
	}
