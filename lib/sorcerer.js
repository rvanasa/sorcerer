...
function sorcerer(config, env, callback)
{
	...
	var context = config.context ? config.context(config) : new sorcerer.Context(config);
	...
	function init(index)
	{
		try
		{
			if(index >= packages.length && callback)
			{
				...
				context.register('env', rootPackage, () => env);
			...
			{
				context.addResource(new Resource(id, pkg, pkg.include[id]), pkg.decorate);
			}
				...
				glob(path.join(basePath, pkg.path, '**/*'), {nodir: true, ignore: pkg.ignore}, (err, files) =>
				{
					...
					try
					{
						for(var i = 0; i < files.length; i++)
						{
							...
							{
								context.register(name, pkg, handle, pkg.decorate);
							}
							sorcerer.loadingInfo = null;
						}
						init(index + 1);
...
sorcerer.define = function(config, handle)
{
	...
	return sorcerer.loadingInfo.context.register(config.name || sorcerer.loadingInfo.name, sorcerer.loadingInfo.pkg, handle, config.decorate);
}
