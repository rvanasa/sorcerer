...
class Context
{
	...
	addResource(resource)
	{
		...
		{
			this.log('Overwriting resource: `' + resource.id + '`');
		}
...
function sorcerer(config, callback)
{
	...
	function init(index)
	{
		try
		{
			...
			{
				context.log('Registering utilities:');
...
sorcerer.define = function(config, handle)
{
	...
	return sorcerer.loadingInfo.context.register(config.name || sorcerer.loadingInfo.name, handle);
}
